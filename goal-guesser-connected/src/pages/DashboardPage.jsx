...
const RANK_MEDAL = {
  1: { label: '👑', color: 'linear-gradient(135deg,#FFD700,#FFA500)', textColor: '#000', glow: '0 0 20px rgba(255,215,0,0.4)' },
  2: { label: '🥈', color: 'linear-gradient(135deg,#C0C0C0,#A0A0A0)', textColor: '#000', glow: '0 0 20px rgba(192,192,192,0.3)' },
  3: { label: '🥉', color: 'linear-gradient(135deg,#CD7F32,#B8681A)', textColor: '#fff', glow: '0 0 20px rgba(205,127,50,0.3)' },
};
      ...
      <div className="relative z-10">
        ...
        <main className="max-w-5xl mx-auto px-4 py-6">
          ...
          {currentUser && userEntry && (
            ...
            >
              {[
                { label: 'My Rating',   value: currentUser.rating,      icon: '⭐', color: userTier?.color },
                { label: 'Wins',        value: currentUser.wins,        icon: '🏆', color: '#10b981' },
                { label: 'Win Rate',    value: `${winRate}%`,           icon: '📊', color: '#00d4ff' },
                { label: 'Best Streak', value: currentUser.bestStreak ?? currentUser.streak, icon: '🔥', color: '#f59e0b' },
              ].map(({ label, value, icon, color }) => (
                <div key={label} className="bg-pitch-700 border border-neon-cyan/15 rounded-2xl p-4 text-center">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="font-orbitron font-black text-xl gradient-text">🏆 LEADERBOARD</h2>
                <p className="text-slate-500 text-sm mt-0.5">Top players ranked by rating</p>
              </div>
              <div className="text-xs text-slate-600 font-orbitron">{rankedBoard.length} players</div>
            </div>
              ...
              <div className="bg-pitch-700 border border-neon-cyan/20 rounded-2xl overflow-hidden">
                <div className="grid grid-cols-[56px_1fr_100px_80px] gap-2 px-5 py-3 bg-pitch-600/60 border-b border-neon-cyan/10">
                  {['Rank', 'Player', 'Rating', 'Games'].map((h, i) => (
                    <p key={h} className={`text-[10px] uppercase tracking-widest text-slate-500 font-semibold ${i >= 2 ? 'text-right' : ''}`}>
                      {h}
                    </p>
                  ))}
                </div>
                ...
                {rows.map((row, idx) => {
                  if (row === 'divider') {
                    return (
                      <div key="divider" className="px-5 py-2 text-center text-slate-700 text-xs border-b border-neon-cyan/5 font-orbitron tracking-widest">
                        • • •
                      </div>
                    );
                  }
                  ...
                  return (
                    <motion.div
                      ...
                      style={{
                        background: medal
                          ? isMe ? `${ROW_BG[user.rank]}, rgba(124,58,237,0.06)` : ROW_BG[user.rank]
                          : isMe ? 'rgba(124,58,237,0.07)' : undefined,
                    ...
                    >
                      ...
                      <div className="flex items-center">
                        <div
                          ...
                          style={medal
                            ? { background: medal.color, color: medal.textColor, boxShadow: medal.glow }
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className={`font-semibold text-sm truncate ${isMe ? 'text-neon-purple' : 'text-slate-200'}`}>
                            {user.username}
                            {isMe && <span className="ml-1.5 text-[10px] text-neon-purple/70 font-normal">(you)</span>}
                          </p>
                      ...
                      {/* Games */}
                      <div className="flex items-center justify-end">
                        <span className="text-slate-500 text-sm">{user.games}</span>
                      </div>
                    </motion.div>
                  );
                })}
