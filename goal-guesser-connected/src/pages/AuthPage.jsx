...
const AuthPage = () => {
  ...
  useEffect(() => { if (currentUser) navigate('/game'); }, [currentUser, navigate]);
      ...
      >
        <div className="card p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 text-sm mt-1">Guess the football legend</p>
          </div>
          ...
          <div className="flex bg-pitch-900 rounded-xl p-1 mb-6">
            {['login', 'register'].map((t) => (
              <button
                ...
                style={tab === t ? { background: 'linear-gradient(135deg,#00d4ff,#7c3aed)' } : {}}
              >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] text-slate-500 uppercase tracking-widest mb-2">Username</label>
                  <input className="input-field" placeholder="Enter username" value={username} onChange={(e) => setUsername(e.target.value)} />
                </div>
                ...
                {tab === 'register' && (
                  <div>
                    ...
                    <input className="input-field" placeholder="your@email.com" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
                  </div>
                )}

                <div>
                  <label className="block text-[10px] text-slate-500 uppercase tracking-widest mb-2">
                    Password{tab === 'register' && <span className="text-slate-600 ml-1 normal-case tracking-normal">(min 6 chars)</span>}
                  </label>
                  <input
                ...
                >
                  {authLoading
                    ? <span className="inline-flex items-center gap-2"><span className="animate-spin">⚽</span> Loading...</span>
          ...
          {tab === 'login' && (
            <p className="text-center text-xs text-slate-600 mt-4">
              Demo: <span className="text-slate-400">demo</span> / <span className="text-slate-400">pass1234</span>
            </p>
          )}
        </div>
